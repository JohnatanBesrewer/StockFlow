...
class Postman:
    Addr = namedtuple("Addr", ["street", "house", "apartment"])

    def __init__(self):
        self.delivery_data = []

    def add_delivery(self, street, house, apartment):
        self.delivery_data.append(self.Addr(street, house, apartment))

    def get_houses_for_street(self, street):
        return list(
            dict.fromkeys(
                house.house
                for house in filter(lambda dd: dd.street == street, self.delivery_data)
            )
        )
    ...
    def get_flats_for_house(self, street, house):
        return list(
            dict.fromkeys(
                apart.apartment
                for apart in filter(
                    lambda dd: dd.street == street and dd.house == house,
                    self.delivery_data,
                )
            )
        )
